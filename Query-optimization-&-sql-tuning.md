Performed a comprehensive SQL tuning process to optimize inefficient queries. This process involved two main strategies: Logic Refactoring (SQL Tuning) and Physical Tuning (Indexing). I validated the results using AWR (Automatic Workload Repository) reports to measure improvements in throughput and system resource efficiency

Core Objectives:
- Reduce Logical Reads: Lower the CPU overhead by minimizing data block access.
- Eliminate Full Table Scans: Transition from costly table scans to efficient index scans.
- Scalability Validation: Ensure the database maintains high performance even with large volumes of data.
...
2. Generated a large volume of dummy data to evaluate query scalability and observe the optimizer's behavior with high data volumes
...
5. Analyze Execution Plan used DBMS_XPLAN.DISPLAY_CURSOR to inspect the actual execution plan and identify inefficiencies
...
6. Verified through the AWR report that the inefficient query caused high Logical Reads and Full Table Scans
...
8. Validated Refactoring Results Analysis showed a 72% drop in rows/bytes processed and a 50% drop in CPU usage, but the "Full Table Scan" remained
...
9. The AWR report confirmed that logical reads dropped to 31% and table scans to 32% after refactoring the syntax
...
10. Created a B-Tree Index to allow the database to access data directly from the buffer cache rather than scanning the entire disk
...
11. Final Execution Plan Validated The results showed a significant breakthrough
...
12. The final AWR report confirmed a high-performance state, with logical reads dropping to just 9% and table scans to 10%
