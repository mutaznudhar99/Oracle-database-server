Performed a database upgrade from version 12c to 19c on a standalone server using Database Upgrade Assistant (DBUA). This process included pre-upgrade checks and post-upgrade validations to ensure system stability and data integrity.
...
1. Verified the existing 12c database is running and healthy
...
2. Inspected the oratab file to ensure the target old database version is active on the server
...
3. Created directories for pre-upgrade fixup logs, following Optimal Flexible Architecture (OFA) standards
...
6. Purged the recycle bin and created a Guaranteed Restore Point with sql statement. This allows the database to flash back to the 12c state if a disaster happens    during the upgrade
...
8. Reviewed the pre-upgrade SQL and logs to identify any errors or mandatory manual fixups
...
9. Solved all identified issues to prevent the DBUA process from fail midway
...
12. Monitored the upgrade until it 100% completion without errors
...
13. Validated that the oratab library has been automatically updated to the new 19c Oracle Home
...
14. Verified that the sample data created in step 5 is still present and complete
...
16. Once the upgrade was confirmed successful, dropped the restore point to reclaim disk space
...
17. Updated the COMPATIBLE initialization parameter to 19.0.0. This enables new 19c features
