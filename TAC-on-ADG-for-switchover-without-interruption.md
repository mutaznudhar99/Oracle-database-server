Implemented Transaction Application Continuity (TAC) on Oracle Active Data Guard environment. This configuration ensures the database transactions still uninterrupted during a failover or switchover, providing a seamless sessions for the application layer by automatically replaying in-flight transactions on the new primary database
...
2. Created and configured a dedicated database service with TAC parameters to handle transaction continuity
...
4. Verified TAC Parameters used SQL*Plus to query and confirm the Transaction Application Continuity parameters are correctly registered within the database          instance
...
5. Verified the Data Guard replication is synchronized and both instances are ready for a role transition
...
6. Performed a data injection (INSERT) without a COMMIT. This puts the transaction in a "hang down" situation to test the replay capability
...
7. Initiated a database switchover. During this phase, the application session pauses as TAC start replaying the uncommitted transaction to the new primary           database
...
8. Confirmed the transaction was successfully committed after the switchover was completed, without any session errors or interruptions
...
9. Verified the database roles have successfully exchange, while the application session still active and connected throughout the process
