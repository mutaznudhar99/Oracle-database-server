...
1. Confirmed the Oracle Clusterware (Grid Infrastructure) and listeners are online and healthy all RAC nodes
...
3. Validated TAC Parameters used SQL*Plus to query the database metadata, ensuring the TAC parameters (FAILOVER_TYPE=AUTO) are active on the RAC instance
...
4. Configured Connection String (TNS) using the SCAN IP and the TAC-enabled service name to interconnect the application with the RAC clusterware
...
5. Configured the application (Swingbench) with the TAC connection string to test workload continuity
...
6. Executed a DML operation (INSERT) without a COMMIT to test if the database can replay "in-flight" transaction if a failure happens
...
7. Shut down one RAC node 1 and try at the same time to commit the transaction from the application. Verified that the transaction successfully replayed              and committed on the available node without error   
...
8. Tested a standard connection string without TAC features. Confirmed when a node fails in this scenario, the application immediately receives a "Connection         Lost" or "Communication Channel" error.
