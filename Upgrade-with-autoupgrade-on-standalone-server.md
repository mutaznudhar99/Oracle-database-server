...
- Automated Detection: It automatically detects errors and performs fixups.
...
- Speed & Modernity: It is faster and is the currently recommended method by Oracle.
...
3. Verified that the current user environment is pointing to the old (12c) database
...
6. Confirmed that the baseline data is not yet present in the new version before the upgrade starts
...
8. Created configuration file required to move the data from Non-CDB to a Pluggable Database (PDB)
...
11. Reviewed Analyze report the generated HTML documentation to ensure no errors were found during the pre-checks
...
13. Monitored the log files in real-time to ensure no critical errors or bottlenecks occurred
...
15. Verified the 12c background processes are no longer running and have been replaced by the 19c processes
...
17. Validated the baseline data in the new PDB to ensure that all data was migrated successfully with Zero Data Loss
