demonstrated a physical backup and recovery process using Oracle’s native tool, Recovery Manager (RMAN). In this simulation, I tested system resilience by intentionally deleting critical database files from Automatic Storage Management (ASM) and resolving the failure using RMAN’s Data Repair Advisor to minimize downtime
...
2. Verified all cluster resources used the crsctl utility were online. RMAN requires Oracle Cluster Registry (OCR) and ASM to be running to function properly
...
6. Verified backup metadata used the list backup command to ensure all crucial components were successfully saved to the designated Disk Group
...
7. Verified the ASM instance to confirm that the physical backup files were correctly created and registered
...
9. Attempted to query the database in SQL*Plus. The query failed because the background processes (DBWR/LGWR) could not find the missing datafile
...
10. Did recovery database used the Data Repair Advisor (list failure, advise failure, repair failure) to automatically diagnose the problem and restore the            missing files  
...
11. Verified the datafiles again to ensure they were back ONLINE and that the database instance was in an OPEN stat
...
12. Successfully ran a query statement to confirm that the database could read the restored datafiles and that the system was fully recovered
