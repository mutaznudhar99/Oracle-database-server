Demonstrated a comprehensive security strategy using User Profiles and Role-Based Access Control (RBAC). This approach serves as an instrument for access control, security enforcement, and performance optimization through effective resource management


Purpose:
- Security Hardening: Tighten database security through strict password policies.
- Resource Management: Control and limit system resource consumption per user.
- Risk Mitigation: Reduce the risk of unauthorized access by following the Principle of Least Privilege.


1. Defined profile parameters to manage user behavior and enforce password security policies
...
2. Verified Profile Configuration used query statements to ensure the new profiles were correctly added to the database instance
...
3. Verified that the password verification functions meet minimum security standards using administrative queries
...
4. Created roles to act as containers for operational privileges. This allows for easier collective maintenance of permissions based on job functions 
...
5. Did tested the password policy by attempting to create a user with a password that does not meet the complexity criteria
...
6. Created a new user account, assigning the appropriate security profile and setting disk resource limits (tablespace quotas)
...
7. Assigned the RBAC roles to the user to provide the necessary access for their specific tasks
...
9. Conducted a "penetration test" by attempting to execute commands outside of the assigned privileges to confirm that unauthorized actions are blocked
...
10. Did tested multiple failed login attempts to trigger the FAILED_LOGIN_ATTEMPTS limit until the account status changed to LOCKED
...
11. Did unlocked the user and restore operational accessibility used a superuser (DBA)
